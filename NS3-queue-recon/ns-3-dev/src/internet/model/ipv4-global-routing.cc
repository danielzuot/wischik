...
Ipv4GlobalRouting::GetTupleValue (const Ipv4Header &header, Ptr<const Packet> ipPayload)
{
  ...
  switch (header.GetProtocol ())
    ...
    case UDP_PROT_NUMBER:
      {
        /////////////////////////////////////////////////////////////////
        //Don't forget to change this value if you change UDP echo port!
        uint16_t UDP_echo_port = 10;
        /////////////////////////////////////////////////////////////////
        UdpHeader udpHeader;
        ...
        uint8_t *udpPktData = new uint8_t[100];
        ipPayload->CopyData(udpPktData,100);
//        std::cout<<header.GetSource ().Get ()<<" "<<udpHeader.GetSourcePort ()<<" "<<header.GetDestination ().Get ()<<" "<<udpHeader.GetDestinationPort ()<<" "<<(int)header.GetProtocol ()<<" "<<udpPktSize<<std::endl;
//        for (int i=0; i<udpPktSize; i++)
//          {
//            std::printf("%u ",udpPktData[i]);
//          }
//        std::cout<<std::endl;

//        std::cout << (uint32_t) header.GetTtl() << std::endl;
        if (udpHeader.GetSourcePort () != UDP_echo_port) //If this UDP pkt is NOT an echo pkt
          {
            tupleValue += (uint8_t) udpPktData[16 + 64-header.GetTtl()]; //Add route marker0 into tupleValue. Index = 8bytes(UDP header) + 8bytes(Seq number) + the (64-TTL)th byte in marker0 region
          }
        else //If this UDP pkt is an echo pkt
          {
            tupleValue += (uint8_t) udpPktData[28 + 64-header.GetTtl()]; //Add route marker1 into tupleValue. Index = 8bytes(UDP header) + 8bytes(Seq number) + 12bytes(marker0) + the (64-TTL)th byte in marker1 region
          }
    ...
    case TCP_PROT_NUMBER:
      {
        ...
        uint8_t *tcpPktData = new uint8_t[40];
        uint32_t tcpPktDataSize = ipPayload->CopyData(tcpPktData,40);
//        std::cout<<header.GetSource ().Get ()<<" "<<tcpHeader.GetSourcePort ()<<" "<<header.GetDestination ().Get ()<<" "<<tcpHeader.GetDestinationPort ()<<" "<<(int)header.GetProtocol ()<<" "<<tcpPktDataSize<<std::endl;
//        for (uint32_t i=0; i<tcpPktDataSize; i++)
//          {
//            std::printf("%u ",tcpPktData[i]);
//          }
//        std::cout<<std::endl;
        if (tcpPktDataSize >= 39) //If the length of ipPayload >= 20 Bytes(tcp header) + 11 Bytes(path markers), and the length is large enough that this pkt shouldn't be the request pkt (i.e. payload is the requested data size)
          {
            tupleValue += (uint8_t) tcpPktData[20 + 64-header.GetTtl()]; //Add path marker into tupleValue. Index = 20bytes(TCP header) + the (64-TTL)th byte in path marker region
          }
        delete []tcpPktData;
        break;
      }
...
bool 
compareInterface(const Ipv4RoutingTableEntry* m1, const Ipv4RoutingTableEntry* m2){
     return m1->GetInterface () < m2->GetInterface ();
}
...
Ipv4GlobalRouting::LookupGlobal (const Ipv4Header &header, Ptr<const Packet> ipPayload, Ptr<NetDevice> oif)
{
  ...
  NS_LOG_DEBUG ("Looking for route for destination " << header.GetDestination());
      ...
      if ((*i)->GetDest ().IsEqual (header.GetDestination ())) 
        {
          ...
          NS_LOG_DEBUG (allRoutes.size () << "Found global host route" << *i); 
        }
    }
          ...
          if (mask.IsMatch (header.GetDestination (), entry)) 
            {
              ...
              NS_LOG_DEBUG (allRoutes.size () << "Found global network route" << *j);
            }
        }
    }
              ...
              if (oif != 0)
                {
                  if (oif != m_ipv4->GetNetDevice ((*k)->GetInterface ()))
                    {
                      NS_LOG_DEBUG ("Not on requested interface, skipping");
                      continue;
                    }
                }
  ...
  if (allRoutes.size () > 0 ) // if route(s) is found
    {
      ...
      if (m_randomEcmpRouting)
      ...
      else  if (m_flowEcmpRouting && (allRoutes.size () > 1))
        {
          ...
          NS_LOG_DEBUG ("allRoutes.size=" << allRoutes.size () << "; selectIndex=" << 
                       selectIndex);
        }
      else
        {
      ...
      if (allRoutes.size () > 1 )
        {
          // Sort the possible routes by the interface num
          std::sort(allRoutes.begin(), allRoutes.end(), compareInterface);
        }

/*
if (allRoutes.size () > 13 )
{
      NS_LOG_DEBUG ("Debug info " << allRoutes.at (0)->GetInterface ()<<" "<<
allRoutes.at (1)->GetInterface ()<<" "<<allRoutes.at (2)->GetInterface ()<<" "<<allRoutes.at (3)->GetInterface ()<<" "<<allRoutes.at (4)->GetInterface ()<<" "<<
allRoutes.at (5)->GetInterface ()<<" "<<allRoutes.at (6)->GetInterface ()<<" "<<allRoutes.at (7)->GetInterface ()<<" "<<allRoutes.at (8)->GetInterface ()<<" "<<
allRoutes.at (9)->GetInterface ()<<" "<<allRoutes.at (10)->GetInterface ()<<" "<<allRoutes.at (11)->GetInterface ()<<" "<<allRoutes.at (12)->GetInterface ()<<" " );
}
*/
