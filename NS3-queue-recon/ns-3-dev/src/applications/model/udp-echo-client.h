...
public:
  ...
  void SetSrcDstAddr (int srcAddr0, int srcAddr1, int dstAddr0, int dstAddr1);
  void SetNetworkParam (uint32_t level_num, uint32_t rack_size, uint32_t rack_num, uint32_t l2_size, uint32_t l2_num, uint32_t l3_size, uint32_t l3_num, uint32_t l4_size, uint32_t l4_num, uint32_t l5_size);
  void SetRTTinfoStream (Ptr<OutputStreamWrapper> streamRTTinfo);
...
private:
  ...
  uint32_t m_count;
  Time m_interval;
  Time m_reconInterval;
  uint32_t randomseed;
//  double *m_randominterval;
  uint32_t m_size;

//  uint8_t marker0[11]; //Max # of hops = 11 for probe pkt
//  uint8_t marker1[11]; //Max # of hops = 11 for echo pkt
  uint8_t **marker0; //Max # of hops = 11 for probe pkt
  uint8_t **marker1; //Max # of hops = 11 for echo pkt
  uint32_t num_pktInReconInterval;
  ...
  int m_srcAddr0, m_srcAddr1, m_dstAddr0, m_dstAddr1; //m_srcAddr0 & m_dstAddr0 are index of rack, m_srcAddr1 & m_dstAddr1 are index of server in this rack
  uint32_t m_level_num; // Number of levels of the network, <=5
  uint32_t m_rack_size, m_rack_num, m_l2_size, m_l2_num, m_l3_size, m_l3_num, m_l4_size, m_l4_num, m_l5_size;
  Ptr<OutputStreamWrapper> m_streamRTTinfo;
};
