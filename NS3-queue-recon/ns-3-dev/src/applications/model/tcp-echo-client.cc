...
TcpEchoClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpEchoClient")
    ...
    .AddAttribute ("PacketSize", "Size of echo data in outbound packets",
                   ...
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("RequestDataSize", "Size of data that the client requests (in Bytes)",
                   UintegerValue (10000),
                   MakeUintegerAccessor (&TcpEchoClient::SetRequestDataSize,
                                         &TcpEchoClient::GetRequestDataSize),
                   MakeUintegerChecker<uint32_t> ())
...
void
TcpEchoClient::SetRequestDataSize (uint32_t requestDataSize)
{
  NS_LOG_FUNCTION (requestDataSize);

  m_requestDataSize = requestDataSize;
}

uint32_t
TcpEchoClient::GetRequestDataSize (void) const
{
  NS_LOG_FUNCTION_NOARGS ();
  return m_requestDataSize;
}
...
void 
TcpEchoClient::SetFlowInfoStream (Ptr<OutputStreamWrapper> streamWrapper)
{
  m_flowInfoStream = streamWrapper;
}
...
TcpEchoClient::Send (void)
{
  ...
  //Put the data size that the client requests into the packet payload
  char pktpayload[20];
  sprintf (pktpayload, "%lu", (unsigned long)m_requestDataSize);
  SetFill(pktpayload);
  ...
  while (packet = socket->RecvFrom (from))
    {
      ...
      NS_LOG_INFO ("Received " << packet->GetSize () << " bytes from " <<
             InetSocketAddress::ConvertFrom (from).GetIpv4 () << " Total "<<m_bytesRecvBack<<" Bytes.");
    }

  if (m_bytesRecvBack >= m_requestDataSize)
    {
      Address self;
	  socket->GetSockName(self);
	  socket->GetPeerName(from);
	  std::ostream *output = m_flowInfoStream->GetStream();
	  //IP address of requestor, Port of requestor, IP address of server, Port of server, Flow length (Bytes), Flow start time, Flow completion time
	  *output<<InetSocketAddress::ConvertFrom (self).GetIpv4 ().Get ()<<" "<<InetSocketAddress::ConvertFrom (self).GetPort ()<<" "<<InetSocketAddress::ConvertFrom (from).GetIpv4 ().Get ()<<" "<<InetSocketAddress::ConvertFrom (from).GetPort ()<<" "<<m_requestDataSize<<" "<<(int)(m_startTime.GetSeconds() * 1e9 + 0.5) <<" "<<(int)(Simulator::Now().GetSeconds() * 1e9 + 0.5)<<std::endl;
