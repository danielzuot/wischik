  ...
  while ((packet = socket->RecvFrom (from)))
    {
      ...
      double pktRxTime = Simulator::Now().GetSeconds();
      packet->RemoveAtEnd (24);
      uint8_t newPayload[24];
      memcpy ( newPayload, &pktRxTime, 8 );
      memset ( newPayload+8, 0, 16 );
      Ptr<Packet> tmpPacket = Create<Packet> (newPayload, 24);
      packet->AddAtEnd (tmpPacket);
