...
private:
  ...
  std::list<uint32_t> m_fromIPAddr, m_fromPort, m_remainDataSize; // List of how much data is remained to be sent for each socket (i.e. each source IP address & port)
  std::list<uint8_t*> m_path; // Path (list of "markers") of data pkts for each socket
