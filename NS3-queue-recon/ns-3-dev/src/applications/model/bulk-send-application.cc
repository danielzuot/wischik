...
void BulkSendApplication::SendData (void)
{
  ...
  if (m_totBytes == m_maxBytes && m_connected)
    {
      int socketCloseSucceed = -1;
      while (socketCloseSucceed == -1)
      {
        socketCloseSucceed = m_socket->Close ();
      }
//      std::cout<<m_totBytes<<" "<<m_startTime.GetSeconds()<<" "<<Simulator::Now().GetSeconds()<<std::endl;
      m_connected = false;
    }
}
