...
ApplicationContainer
TcpEchoClientHelper::Install (Ptr<Node> node, Ptr<OutputStreamWrapper> streamWrapper)
{
  m_client = m_factory.Create<TcpEchoClient> ();
  m_client->SetFlowInfoStream (streamWrapper);
  node->AddApplication (m_client);

  return ApplicationContainer (m_client);
}
