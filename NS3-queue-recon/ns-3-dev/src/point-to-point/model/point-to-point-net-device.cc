...
PointToPointNetDevice::TransmitStart (Ptr<Packet> p)
{
  ...
  if (m_logTxTime)
  {
    //The last 32 bytes work as follows:
    //First 8 bytes: the timestamp (in double) when this packet is sent as probe
    //Second 8 bytes: the timestamp (in double) when this packet is received as probe
    //Third 8 bytes: the timestamp (in double) when this packet is sent as echo
    double pktTxTime = Simulator::Now().GetSeconds();
    uint8_t originalPkt[2000];
    uint32_t originalPktLength = p->CopyData(originalPkt, 2000);
    //!!! Must change here if the size of UDP probe pkt payload is changed!!!
    uint32_t probePktPayloadLength = 64;
    if (originalPktLength == (probePktPayloadLength + 30)) //UDP probe pkt payload length = 64 bytes. It doesn't matter if a TCP data pkt happens to have the same length
    {
      bool isFirstTimestamp = true; //If this timestamp is the first timestamp added on this UDP probe pkt
      for (uint32_t i=probePktPayloadLength+30-32; i<probePktPayloadLength+30-24; i++)
      {
        if (originalPkt[i] != 0)
        {
      ...
      if (isFirstTimestamp) // This timestamp is the first timestamp added on this UDP probe pkt
      {
        p->RemoveAtEnd (32);
        uint8_t newPayload[32];
        memcpy ( newPayload, &pktTxTime, 8 );
        memset ( newPayload+8, 0, 24 );
        Ptr<Packet> tmpPacket = Create<Packet> (newPayload, 32);
        p->AddAtEnd (tmpPacket);
        //std::cout<<"Probe: "<<pktTxTime<<std::endl;
      }
      else // This timestamp is NOT the first timestamp added on this UDP probe pkt
      {
        p->RemoveAtEnd (16);
        uint8_t newPayload[16];
        memcpy ( newPayload, &pktTxTime, 8 );
        memset ( newPayload+8, 0, 8 );
        Ptr<Packet> tmpPacket = Create<Packet> (newPayload, 16);
        p->AddAtEnd (tmpPacket);
        //std::cout<<"Echo: "<<pktTxTime<<std::endl;
      }
    }
  }
...
void
PointToPointNetDevice::SetLogTxTime (bool x)
{
  m_logTxTime = x;
}

bool
PointToPointNetDevice::GetLogTxTime (void)
{
  return m_logTxTime;
}
