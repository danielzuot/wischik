...
delayMat = numpy.matrix(delayMat)
print delayMat[:,0].size
print delayMat[0,:].size

#U, s, V = numpy.linalg.svd((delayMat.T) * delayMat)
U, s, V = numpy.linalg.svd(delayMat)
print s

figure()
plot(range(1,1+len(s)), s, linestyle='', marker='*')
xlim([0, queue_total_num])
